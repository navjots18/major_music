...
def song_page(request, song_id):
	...
	if request.GET:
		...
		if 'reviewed' in request.GET:
			...
			allReviews = SongReview.objects.filter(songId=song_id).order_by('-time')
			average = allReviews.aggregate(Avg('rating'))
			return render('song.html',{'song_pg': song_pg, 'youtubeParsed': youtubeParsed, 'theUser': theUser, 'average': average, "allReviews": allReviews, 'album_pg': album_pg, 'artist_pg': artistData})
	else:
		return render('song.html',{'song_pg': song_pg, 'youtubeParsed': youtubeParsed, 'theUser': theUser, 'average': average, "allReviews": allReviews, 'album_pg': album_pg, 'artist_pg': artistData})
...
def album_page(request, album_id):
	...
	if request.GET:
		...
		if 'reviewed' in request.GET:
			...
			allReviews = AlbumReview.objects.filter(albumId=album_id)
			average = allReviews.aggregate(Avg('rating'))
			return render('album.html',{'album_pg': album_pg, 'theUser': theUser, 'average': average, 'allReviews': allReviews, 'musicData': musicData, 'artist_pg': artistData})
	else:
		return render('album.html',{'album_pg': album_pg, 'theUser': theUser, 'average': average, 'allReviews': allReviews, 'musicData': musicData, 'artist_pg': artistData})
...
def artist_page(request, artist_id):
	...
	if request.GET:
		...
		if 'reviewed' in request.GET:
			...
			allReviews = ArtistReview.objects.filter(artistId=artist_id)
			average = allReviews.aggregate(Avg('rating'))
			return render('artist.html', {'artist_pg': artist_pg, 'average': average, 'theUser': theUser, 'allReviews': allReviews, 'songData': songData, 'albumData': albumData})
	else:
		return render('artist.html', {'artist_pg': artist_pg, 'average': average, 'theUser': theUser, 'allReviews': allReviews, 'songData': songData, 'albumData': albumData})
...
def list_reviews(request):
	...
	return render('list_reviews.html',{'songReview_pg': songReview_pg, 'albumReview_pg': albumReview_pg, 'artistReview_pg': artistReview_pg})
...
def recommendations(request):
	...
	return render('recommendations.html',{'theRecommendedSongs': theRecommendedSongs})
...
def new_releases(request):
	...
	return render('new_releases.html',{'newAlbums': newAlbums})
